...
规则链管理遵循设备管理的资源导航模式：先进入规则链列表，再进入单条规则链详情；新建和编辑使用独立的画布路由。规则链 UUID 通过 TanStack Router 的路径参数传递，不使用查询参数承载编辑资源。

```text
/rule-engine/rule-chains              列表
        ├── /new                      新建
        ├── /$uuid                    详情（只读）
        └── /$uuid/edit               编辑（React Flow 画布）
```

列表负责分页、搜索、状态筛选、空态和行级操作；详情负责展示规则链元数据及只读图预览，并提供编辑、删除和返回列表入口；编辑页负责加载草稿、修改画布和保存。新建保存成功后进入该规则链详情页。列表、详情和编辑共享后端规则链 API 与节点定义目录，但不把三种页面状态揉进同一个组件。
