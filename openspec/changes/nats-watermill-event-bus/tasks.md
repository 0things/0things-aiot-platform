...
- [ ] 2.3 在 `data-engine` 中引入 `pkg/event`，启动针对 `TopicOTAUpgradeCommand`（调度 MQTT 下发）与 `TopicOTAProgressReport`（原子更新数据库状态）的泛型订阅消费协程，并验证单元测试。

## 3. 配置文件与全局集成验证

- [ ] 3.1 更新各微服务配置文件（`backend`、`mqtt-transport`、`data-engine` 的 `config.example.yml` 和 `local.yml`），增加 `event` NATS JetStream 配置。
- [ ] 3.2 运行根目录 `go mod tidy`、`go work sync`、`make test && make build`，确保所有服务编译与单元测试 100% 通过。
