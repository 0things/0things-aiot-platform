...
// Producer 封装 Kafka 生产者，支持按消息类型动态分流至专属 Kafka Topic。
type Producer struct {
	client *kgo.Client
	logger *zap.Logger
}

// NewProducer 初始化 CoAP Transport 的 Kafka 生产者。
func NewProducer(config *viper.Viper, logger *zap.Logger) (*Producer, func(), error) {
	brokers := config.GetStringSlice("kafka.brokers")
	if len(brokers) == 0 {
		logger.Warn("kafka brokers not configured, running in mock mode")
