...
func setupRuleFullErrRouter(mockService *mock_service.MockRuleServiceInterface) *gin.Engine {
	gin.SetMode(gin.TestMode)
	router := gin.New()
	h := &handler.Handler{}
...
func TestRuleHandler_GetRule_NotFound(t *testing.T) {
	...
	router := setupRuleFullErrRouter(mockService)
...
func TestRuleHandler_GetRule_InvalidID(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	mockService := mock_service.NewMockRuleServiceInterface(ctrl)
	router := setupRuleFullErrRouter(mockService)

	req, _ := http.NewRequest("GET", "/rules/abc", nil)
	w := httptest.NewRecorder()
	router.ServeHTTP(w, req)
	assert.Equal(t, http.StatusInternalServerError, w.Code)
}
...
func TestRuleHandler_CreateRule_InvalidJSON(t *testing.T) {
	...
	req, _ := http.NewRequest("POST", "/rules", bytes.NewBufferString("invalid"))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	router.ServeHTTP(w, req)
	assert.Equal(t, http.StatusInternalServerError, w.Code)
...
func TestRuleHandler_UpdateRule_GetFails(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	mockService := mock_service.NewMockRuleServiceInterface(ctrl)
	router := setupRuleFullErrRouter(mockService)
...
func TestRuleHandler_UpdateRule_InvalidJSON(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	mockService := mock_service.NewMockRuleServiceInterface(ctrl)
	router := setupRuleFullErrRouter(mockService)

	req, _ := http.NewRequest("PUT", "/rules/1", bytes.NewBufferString("invalid"))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	router.ServeHTTP(w, req)
	assert.Equal(t, http.StatusInternalServerError, w.Code)
...
func TestRuleHandler_RuleAction_InvalidID(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	mockService := mock_service.NewMockRuleServiceInterface(ctrl)
	router := setupRuleFullErrRouter(mockService)

	req, _ := http.NewRequest("POST", "/rules/abc:enable", nil)
	w := httptest.NewRecorder()
	router.ServeHTTP(w, req)
	assert.Equal(t, http.StatusInternalServerError, w.Code)
}

func TestRuleHandler_EvaluateRule_NotFound(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	mockService := mock_service.NewMockRuleServiceInterface(ctrl)
	router := setupRuleFullErrRouter(mockService)

	mockService.EXPECT().Evaluate(gomock.Any(), int64(1)).Return(nil, repository.ErrNotFound)
...
func TestRuleHandler_RuleExecutions_Success(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	mockService := mock_service.NewMockRuleServiceInterface(ctrl)
	router := setupRuleFullErrRouter(mockService)

	mockService.EXPECT().ListExecutions(gomock.Any(), int64(1), 1, 20).Return([]model.RuleExecution{}, int64(0), nil)
