...
func TestProductRepository(t *testing.T) {
	...
	product := &model.Product{ProductKey: "P001", Name: "sensor", CategoryID: &categoryID, Status: "active", OrganizationID: "org-1"}
	...
	require.NoError(t, repo.Create(ctx, &model.Product{ProductKey: "P002", Name: "updated other tenant", OrganizationID: "org-2"}))
	items, total, err = repo.List(ctx, 1, 20, "", "", "updated")
	require.NoError(t, err)
	require.EqualValues(t, 1, total)
	require.Len(t, items, 1)

	items, total, err = repo.List(tenant.WithOrganization(ctx, "org-2"), 1, 20, "", "", "updated")
	require.NoError(t, err)
	require.EqualValues(t, 1, total)
	require.Len(t, items, 1)
}
