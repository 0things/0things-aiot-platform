...
func setupAlertRouterFull(mockService *mock_service.MockAlertServiceInterface) *gin.Engine {
...
func TestAlertHandler_ListWithFilters(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()

	mockService := mock_service.NewMockAlertServiceInterface(ctrl)
	router := setupAlertRouterFull(mockService)

	alerts := []model.Alert{{ID: 1, Summary: "High Temperature"}}
	mockService.EXPECT().List(gomock.Any(), 1, 20, "critical", "high", "D001").Return(alerts, int64(1), nil)

	req, _ := http.NewRequest("GET", "/alerts?status=critical&severity=high&device_key=D001", nil)
...
func TestAlertHandler_AckWithUser(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
...
func TestAlertHandler_ResolveWithUser(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
...
func TestAlertHandler_GetWithTime(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
